"""Locate and load the cade-flower settings file.

Walks upward from CWD looking for `.claude/skills.settings.{env}.json`,
returns the `cade-flower` block (a flat dict: `user`, `password`, `url`).
...
the repo. Sibling skills (`cade-db-queries`, `cade-logfire`, ...) live under
...
DEFAULT_ENV = "prod"
SKILL_KEY = "cade-flower"
REQUIRED_FIELDS = ("user", "password", "url")
...
def load(start: Path | None = None, env: str | None = None) -> dict[str, Any]:
    ...
    block = data[SKILL_KEY]
    missing = [f for f in REQUIRED_FIELDS if not block.get(f)]
    if missing:
        raise KeyError(
            f"{path} '{SKILL_KEY}' block is missing fields: {', '.join(missing)}"
        )
    return block
