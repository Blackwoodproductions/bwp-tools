...
- **Tools** — provided by the `bwp-core` plugin's `dbhub` MCP server, source id `ranking` (full ids: `mcp__plugin_bwp-core_dbhub__<tool>`):
  - `execute_sql_ranking` `{ "sql": "..." }` — read-only, 30 s timeout, hard cap 500 rows. **Add your own `LIMIT` (≤ 100 unless the user needs more)** — the cap is a backstop, not a default.
