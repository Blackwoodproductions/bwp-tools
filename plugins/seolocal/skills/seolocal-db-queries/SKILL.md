...
**The MCP connection is prod only.** If the user says "staging" / "stg" / "local" / "dev", do **not** run it against the `seo` source. Use the repo-local wrapper if the checkout still has it (`python .claude/skills/seolocal-db-queries/scripts/prod_query.py --env stg "..."`); otherwise tell the user non-prod isn't wired.
